...
def get_bloq_doc_cells(bloqdoc: BloqDocSpec, cid_prefix: str) -> List[_Cell]:
    ...
    return [
        _Cell(text=md_doc, cell_id=f'{cid_prefix}.bloq_doc.md', is_py=False),
        _Cell(text=py_import, cell_id=f'{cid_prefix}.bloq_doc.py', is_py=True),
    ]
...
def get_example_instances_cells(bloqdoc: BloqDocSpec, cid_prefix: str) -> List[_Cell]:
    ...
    cells: List[_Cell] = [
        _Cell(
            text='### Example Instances', cell_id=f'{cid_prefix}.example_instances.md', is_py=False
        )
    ]
...
def get_graphical_signature_cells(bloqdoc: BloqDocSpec, cid_prefix: str) -> List[_Cell]:
    ...
    return [
        _Cell(
            ...
            cell_id=f'{cid_prefix}.graphical_signature.md',
            is_py=False,
        ),
        _Cell(text=code, cell_id=f'{cid_prefix}.graphical_signature.py', is_py=True),
    ]
...
def get_call_graph_cells(bloqdoc: BloqDocSpec, cid_prefix: str) -> List[_Cell]:
    ...
    return [
        _Cell(text='### Call Graph', cell_id=f'{cid_prefix}.call_graph.md', is_py=False),
        _Cell(text='\n'.join(code), cell_id=f'{cid_prefix}.call_graph.py', is_py=True),
    ]
...
def render_notebook(nbspec: NotebookSpecV2) -> None:
    ...
    cells = {
        'title_cell': _Cell(
            text='\n'.join(_get_title_lines(nbspec.title, nbspec.module)),
            cell_id='title_cell',
            is_py=False,
        ),
        'top_imports': _Cell(text=_IMPORTS, cell_id='top_imports', is_py=True),
    }
