...
import yaml
from tensorflow_docs.api_generator.doc_generator_visitor import ApiTreeNode, DocGeneratorVisitor
...
def local_definitions_filter_with_exceptions(path, parent, children):
    if path[:2] == ('qualtran', 'dtype'):
        return children
    return local_definitions_filter(path, parent, children)
...
class MyClassPageBuilder(mixin_custom_template('class'), ClassPageBuilder):  # type: ignore[misc]
    ...
    def build_collapsable_aliases(self):
        aliases = self.page_info.aliases
        if not aliases:
            return ''

        return 'aka ' + ', '.join(f'`{x}`' for x in self.page_info.aliases)
...
class MyDocGeneratorVisitor(DocGeneratorVisitor):
    def _get_module_length_score(self, path):
        priority_modules = [
            ('qualtran', 'dtype'),
            ('qualtran', 'simulation', 'tensor'),
...
def generate_ref_docs(reporoot: Path):
    ...
    doc_generator = DocGenerator(
        ...
        callbacks=[
            local_definitions_filter_with_exceptions,
            filter_type_checking,
            filter_type_aliases_in_the_wrong_place,
        ],
        visitor_cls=MyDocGeneratorVisitor,
    ...
    for entry in section:
        if 'path' in entry:
            ...
            if len(entry["section"]) == 1:
                ...
                print(f'Warning: {subtitle} is too nested')
                subsections.append((subtitle, entry["section"]))
                # _write_toc_links(f, entry["section"])
        else:
            raise ValueError(entry)
    return subsections


def write_ref_toc(f, toc):
    ...
    while len(sections) > 0:
        ...
        if defined_order:
            for i, (title, _) in enumerate(sections):
                if title == defined_order[0]:
                    defined_order.pop(0)
                    break
            else:
                raise ValueError(defined_order)
        else:
            i = 0
        ...
        subsections = _write_toc_links(f, title, section)
