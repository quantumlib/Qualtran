...
def filter_type_checking(
    path: tuple[str, ...], parent: Any, children: list[tuple[str, Any]]
) -> list[tuple[str, Any]]:
...
def filter_type_aliases_in_the_wrong_place(
    path: tuple[str, ...], parent: Any, children: list[tuple[str, Any]]
) -> list[tuple[str, Any]]:
...
class MyClassPageBuilder(mixin_custom_template('class'), ClassPageBuilder):  # type: ignore[misc]
    ...
    def __init__(self, page_info: Any) -> None:
...
def generate_ref_docs(reporoot: Path) -> None:
...
def write_ref_toc(f: Any, grouped_paths: dict[Path, list[Path]], output_dir: Path) -> None:
...
def fixup_suffix(content: str) -> str:
    ...
    def _replace_internal_suffixes(match: re.Match[str]) -> str:
...
def remove_extra_files(output_dir: Path) -> None:
...
def apply_fixups(reporoot: Path) -> None:
...
def generate_ref_toc(reporoot: Path) -> None:
