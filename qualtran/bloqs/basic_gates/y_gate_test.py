...
def test_cirq_interop():
    ...
    should_be = cirq.Circuit(
        [cirq.Moment(cirq.Y(cirq.NamedQubit('q')).controlled_by(cirq.NamedQubit('ctrl')))]
    )
...
def test_active_cy_is_y():
    ...
    ctrl_on, q = bb.add(CYGate(), ctrl=ctrl_on, q=q)
...
def test_cy_adjoint():
    ...
    q = bb.add_register('q', 1)
    ctrl, q = bb.add(CYGate(), ctrl=ctrl, q=q)
    ctrl, q = bb.add(CYGate().adjoint(), ctrl=ctrl, q=q)
