...
def test_reflection_using_prepare(num_ones, eps, global_phase):
    data = [1] * num_ones
    prepare_gate = StatePreparationAliasSampling.from_coefficients(data, precision=eps * sum(data))
...
def test_reflection_using_prepare_consistent_protocols_and_controlled():
    prepare_gate = StatePreparationAliasSampling.from_coefficients([1, 2, 3, 4], precision=0.1 * 10)
    # No control
    gate = ReflectionUsingPrepare(prepare_gate, control_val=None)
    op = gate.on_registers(**get_named_qubits(gate.signature))
...
def test_call_graph_matches_decomp(global_phase, control_val):
    data = [1] * 5
    eps = 1e-11
    prepare_gate = StatePreparationAliasSampling.from_coefficients(data, precision=0.01 * sum(data))
