...
class PrepareNuStateWithProj(Bloq):
    ...
    def build_composite_bloq(
        ...
        mu, flag_mu = bb.add(
            PrepareMuUnaryEncodedOneHotWithProj(
                self.num_bits_n, self.num_bits_p, is_adjoint=self.is_adjoint
            ),
            mu=mu,
        )
        mu, nu = bb.add(
            PrepareNuSuperPositionState(self.num_bits_n, is_adjoint=self.is_adjoint), mu=mu, nu=nu
        )
        nu, flag_zero = bb.add(
            FlagZeroAsFailure(self.num_bits_n, is_adjoint=self.is_adjoint), nu=nu
        )
        mu, nu, flag_nu_lt_mu = bb.add(
            TestNuLessThanMu(self.num_bits_n, is_adjoint=self.is_adjoint), mu=mu, nu=nu
        )
        n_m = (self.m_param - 1).bit_length()
        m = bb.add(PrepareUniformSuperposition(self.m_param), target=m)
        mu, nu, m, flag_nu = bb.add(
            TestNuInequality(self.num_bits_n, n_m, is_adjoint=self.is_adjoint),
            mu=mu,
            nu=nu,
            m=m,
    ...
    def build_call_graph(self, ssa: 'SympySymbolAllocator') -> Set['BloqCountT']:
        ...
        # 3. Remove minus zero
        cost_3 = (FlagZeroAsFailure(self.num_bits_n, is_adjoint=self.is_adjoint), 1)
        # 4. Test $\nu < 2^{\mu-2}$
        cost_4 = (TestNuLessThanMu(self.num_bits_n, is_adjoint=self.is_adjoint), 1)
        # 5. Prepare superposition over $m$ which is a power of two so only clifford.
        # 6. Test that $(2^{\mu-2})^2\mathcal{M} > m (\nu_x^2 + \nu_y^2 + \nu_z^2)$
        cost_6 = (TestNuInequality(self.num_bits_n, n_m, is_adjoint=self.is_adjoint), 1)
