...
from qualtran.rotation_synthesis.protocols import clifford_t_synthesis
...
def test_diagonal_protocol(theta, error):
    res = clifford_t_synthesis.diagonal_unitary_approx(theta, error, 100, _CONFIG)
...
def test_fallback_protocol(theta, error):
    res = clifford_t_synthesis.fallback_protocol(
...
def test_mixed_diagonal_protocol(theta, error):
    res = clifford_t_synthesis.mixed_diagonal_protocol(theta, error, 100, _CONFIG)
    assert res is not None
...
def test_mixed_fallback_protocol(theta, error):
    res = clifford_t_synthesis.mixed_fallback_protocol(
