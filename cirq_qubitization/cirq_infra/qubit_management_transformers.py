...
def map_clean_and_borrowable_qubits(
    ...
    Args:
        circuit: Input cirq circuit containing temporarily allocated `CleanQubit`/`BorrowableQubit`s.
        qm: An instance of `QubitManager` specifying the strategy to use for allocating / deallocating
            new ancilla qubits to replace the temporary qubits.

    Returns:
        An updated `cirq.Circuit` with all `CleanQubit`/`BorrowableQubit`s mapped to either existing
        system qubits or new ancilla qubits allocating using the `qm` qubit manager.
    """
    if qm is None:
    ...
    return cirq.Circuit(map_func(op, idx) for idx, m in enumerate(circuit) for op in m)
