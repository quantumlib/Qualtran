...
def test_reflection_using_prepare_diagram():
    ...
    prepare_gate = cq.StatePreparationAliasSampling(data, probability_epsilon=eps)
    # num_controls = 0
    gate = cq.ReflectionUsingPrepare(prepare_gate, num_controls=0)
...
ancilla_6: ────less_than_equal─────────────────────┼───────less_than_equal─────────────────
               │                                   │       │
selection0: ───StatePreparationAliasSampling───X───@───X───StatePreparationAliasSampling───
               │                                   │       │
selection1: ───selection───────────────────────X───@───X───selection───────────────────────
               │                                   │       │
selection2: ───selection^-1────────────────────X───@───X───selection───────────────────────
''',
    )

    # num_controls = 0
    gate = cq.ReflectionUsingPrepare(prepare_gate, num_controls=1)
    op = gate.on_registers(**gate.registers.get_named_qubits())
    circuit = greedily_allocate_ancilla(cirq.Circuit(cirq.decompose_once(op)))
    cirq.testing.assert_has_diagram(
        circuit,
