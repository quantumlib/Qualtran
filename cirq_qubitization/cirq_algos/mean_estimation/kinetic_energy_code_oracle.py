...
class KineticEnergyCodeOracle(SelectOracle):
    ...
    def selection_registers(self) -> cq.SelectionRegisters:
        return cq.SelectionRegisters.build(selection=((3 * self.stopping_system.n_n, 
                                                       2**(3 * self.stopping_system.n_n))))
    ...
    def get_encoded_register_size(self) -> int:
        p_xyz_max = 2**(self.stopping_system.n_n - 1) # minus 1 in exponent is for +- range for 2^nn values
        self._ke_prefactor = 2 * numpy.pi**2 / self.stopping_system.Omega**(2/3) / self.stopping_system.mpr

        self._max_ke = self._ke_prefactor * 3 * p_xyz_max**2
        self._num_bits_kp_max = numpy.floor(numpy.log2(self._max_ke) + 1)

        self._num_bits_kmean = numpy.floor(numpy.log2(self.stopping_system.get_kmean_in_au()**2 
                                                / (2 * self.stopping_system.mpr)
                                                )
                                    ) + 1

        self._num_bits_cross = numpy.floor(numpy.log2(self.stopping_system.get_kmean_in_au() * 
                                                (2 * numpy.pi * p_xyz_max / 
                                                 self.stopping_system.Omega**(1/3)
                                                 ) / self.stopping_system.mpr
                                                 )
                                    ) + 1

        return int(numpy.floor(numpy.log2(3 * 
                                          2**max(self._num_bits_kp_max, 
    ...
    def calculate_toffoli_costs(self) -> int:
        ...
        # step 2
        ke_of_cross = 4 * (2 * self._num_bits_kmean * n_n - self._num_bits_kmean) + 2 * self._num_bits_kmean + 1
