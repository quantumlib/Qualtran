...
class SplitJoin(Bloq):
    ...
    def build_composite_bloq(
        ...
        (xxs,) = bb.add(Split(self.nn), split=xx)
        (xx2,) = bb.add(Join(self.nn), join=xxs)
        return {'xx': xx2}
...
class ModMultiply(Bloq):
    ...
    def build_composite_bloq(
        ...
        (ctls,) = bb.add(Split(self.exponent_bitsize), split=exponent)
        ...
        return {'exponent': bb.add(Join(self.exponent_bitsize), join=ctls)[0], 'x': x}
